...
const CpuProfilerBindings = {
  ...
  stopProfiling(name) {
    ...
    return PrivateCpuProfilerBindings.stopProfiling(name, node_worker_threads.threadId, !!utils.GLOBAL_OBJ._sentryDebugIds);
  },
};
...
const THREAD_ID_STRING = String(node_worker_threads.threadId);
const THREAD_NAME = node_worker_threads.isMainThread ? 'main' : 'worker';
...
,
  ...
  const profile = {
    ...
    runtime: {
      name: 'node',
      version: node_process.versions.node || '',
    },
    os: {
      name: PLATFORM,
      version: RELEASE,
    ...
    device: {
      locale: node_process.env['LC_ALL'] || node_process.env['LC_MESSAGES'] || node_process.env['LANG'] || node_process.env['LANGUAGE'] || '',
      model: MODEL,
      manufacturer: TYPE,
      architecture: ARCH,
      is_emulator: false,
